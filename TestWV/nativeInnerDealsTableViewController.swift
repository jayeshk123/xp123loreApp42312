//
//  nativeInnerDealsTableViewController.swift
//  TestWV
//
//  Created by earth on 06/10/17.
...
class nativeInnerDealsTableViewController: UITableViewController {
            ...
            if let json = response.data {
                ...
                if(data.count > 0){
                    for i in 0..<data.count{
                        ...
                        self.deals.append(Deals(newstitle: Title, excerpt: Excerpt, date: Date, link: Link, image : trimmedString, uniqueID: id, last_updated: Date))
                    }
                }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //if(expandedRowIndex != -1 && indexPath.row == expandedRowIndex){
            let cell = Bundle.main.loadNibNamed("DealsCell", owner: self, options: nil)?.first as! DealsCell
            ...
            cell.titleLabel.text = deals[indexPath.row].newstitle
            cell.addressLabel.text = deals[indexPath.row].link
            cell.mainContent.text = deals[indexPath.row].excerpt
            cell.dateLabel.text = deals[indexPath.row].date
            cell.mainImage.layer.cornerRadius = 20.0
            cell.layer.cornerRadius = 20.0
            let trimmedString = deals[indexPath.row].image
    ...
    override func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        (cell as! DealsCell).watchFrameChanges()
    }
    
    override func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
         (cell as! DealsCell).ignoreFrameChanges()
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        for cell in tableView.visibleCells as! [DealsCell] {
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath == selectedIndexPath {
            return DealsCell.expandedHeight
        } else {
            return DealsCell.defaultHeight
        }
    }
